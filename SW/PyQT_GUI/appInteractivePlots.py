...
def plot_mean_data(timestamps, data_columns, title):
    import numpy as np
    fig, ax = plt.subplots(figsize=(10, 6))

    # Converte tutte le colonne in una matrice numpy per calcolare la media per timestamp
    if not data_columns:
        print("No data columns to plot.")
        return

    data_matrix = np.array([data_columns[key] for key in data_columns])
    mean_values = np.nanmean(data_matrix, axis=0)  # media orizzontale (tra colonne)

    # Plot singola curva con i valori medi
    ax.plot(timestamps, mean_values, label="Mean Value", linestyle='-', marker='o', color='tab:red')
...
def plot_mean_all_days(data_type, remove_erroneous_values):
    ...
    plot_mean_data(timestamps, data_columns, f"Mean Temperature - {data_type}")
...
try:
    if arg1 == 'PLOT_ALL_DAYS_DATA_EXTERNAL_TEMPERATURE':
        plot_all_days('External_Temperature', remove_errors)
